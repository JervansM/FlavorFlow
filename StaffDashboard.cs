...
namespace FlavorFlowIT13
{
    public partial class StaffDashboard : Form
    {
        ...
        private void StaffDashboard_Load(object sender, EventArgs e)
        {
            ...
            RoundPanel(panelContent, 19);
            RoundPanel(panel2, 19);
            RoundPanel(systemsearchbarpanel, 19);
        ...
        private void RestoreOriginalMenuUI()
        {
            if (!string.IsNullOrEmpty(selectedOrderType))
            {
                // If ordering mode is active, show order version of menu
                LoadContent(new StaffDashboardMenuFormOrder(selectedOrderType));
            }
            else
            {
                LoadContent(new StaffDashboardMenuForm());
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            panelContent.Controls.Add(CreateMenuCard(reader));
                        }
                    }
                }
            }
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            // Debugging line: Track how many rows are being read
                            count++;
                            panelContent.Controls.Add(CreateMenuCard(reader));
                        }

                        // Debugging: Log the number of items loaded
                        Console.WriteLine($"Loaded {count} items from the database.");
                    }
                }
            }
            catch (Exception ex)

    ...
    namespace FlavorFlowIT13
    {
        ...
        private void supplierorderbtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("No active database connection detected.", "Connection Error",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                {
                    MessageBox.Show("Please select both a supplier and an item.", "Validation Error",
                ...
                if (!decimal.TryParse(suppliercosttxt.Text, out decimal unitCost) || unitCost <= 0)
                {
                ...
                if (!decimal.TryParse(supplierbillamounttxt.Text, out decimal totalCost) || totalCost <= 0)
                {
                ...
                using (SqlConnection con = new SqlConnection(activeConnectionString))
                {
                    ...
                    using (SqlTransaction transaction = con.BeginTransaction())
                    {
                        ...
                        catch (SqlException sqlEx)
                        {
                            ...
                            string userMessage = "A database error occurred while saving the order.";
                            string devMessage = $"[SQL Error {sqlEx.Number}] {sqlEx.Message}";

                            MessageBox.Show($"{userMessage}\n\n{devMessage}", "Database Error",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        catch (InvalidOperationException invEx)
                        {
                            transaction.Rollback();
                            MessageBox.Show("A connection or transaction error occurred:\n" + invEx.Message,
                                "Transaction Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        catch (Exception ex2)
                        {
                            transaction.Rollback();
                            MessageBox.Show("Unexpected error while processing order:\n" + ex2.Message,
            ...
            {
                MessageBox.Show($"Database connection failed:\n{sqlOuter.Message}", "Connection Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception ex)
            {
